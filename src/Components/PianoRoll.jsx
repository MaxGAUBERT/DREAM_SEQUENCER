...
const PianoRoll = React.memo(({ selectedPatternID, selectedInstrument, instrumentList, setInstrumentList, onOpen, onClose }) => {
    ...
    // Refs pour les données dynamiques
  ...
  useEffect(() => {
    ...
    loopRef.current = new Tone.Loop((time) => {
      ...
      Object.entries(currentInstrumentList).forEach(([_, instrument]) => {
  ...
  useEffect(() => {
    setInstrumentList(prev => {
      const updated = { ...prev };
      for (const instrumentName in updated) {
        const instrument = updated[instrumentName];
        if (!instrument.pianoData) continue;

        const newPianoData = {};
        for (const patternID in instrument.pianoData) {
          const notes = instrument.pianoData[patternID] || [];
          newPianoData[patternID] = notes.filter(n => n.start + n.length <= cols);
        }

        updated[instrumentName] = {
          ...instrument,
          pianoData: newPianoData
        };
      }
      return updated;
    });
    ...
    }, [isResizing, selectedNoteId, resizeDirection, handleSetNotes, mode]);
