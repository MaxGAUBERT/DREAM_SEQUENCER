...
export const useStorage = ({
  ...
  const createProjectData = useCallback(() => {
    ...
    return {
      ...
      playlist: {
        ...
        rows: playlist.rows || defaultRows,
        cols: playlist.cols || defaultCols,
      },
  ...
  }, [channelSources, grids, patterns, playlist, defaultRows, defaultCols]);
  ...
  const handleLoadProject = useCallback((projectName, projectData) => {
    ...
    try {
      ...
      // 4. Charger la playlist
      if (projectData.playlist) {
        console.log('📋 Chargement de la playlist:', projectData.playlist);
        setPlaylist({
          initGrid: projectData.playlist.initGrid || [],
          rows: projectData.playlist.rows || defaultRows,
          cols: projectData.playlist.cols || defaultCols,
        });
        
        // Mettre à jour les dimensions
        if (projectData.playlist.rows) setRows(projectData.playlist.rows);
        if (projectData.playlist.cols) setCols(projectData.playlist.cols);
      }
  ...
  }, [setPatterns, setGrids, setChannelSources, setPlayers, setPlaylist, setRows, setCols, setProjectName, defaultRows, defaultCols]);
