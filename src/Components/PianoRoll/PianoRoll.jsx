...
const PianoRoll = ({ selectedInstrument, onClose }) => {
  ...
  const { getNotes, setNotes, addNote, removeNote, updateNote, clearNotes, filterNotesInRange } =
    usePianoRollNotes(selectedInstrument, selectedPatternID);

  const { registerStepSetter } = usePianoRollAudio(numSteps);
  ...
  // ── Zoom ──────────────────────────────────────────────────────────────────
  const naturalWidthPx    = numSteps * CELL_WIDTH;
  ...
  }, [setNumSteps, filterNotesInRange, setWindowRange]);
  ...
  const handleMouseMove = useCallback((e) => {
    if (isResizingLocal && initialNote) {
      ...
      setNotes((prev) => prev.map((n) => {
        ...
        if (resizeMode === "right") {
          return { ...n, length: Math.max(1, Math.min(initialNote.length + deltaCells, numSteps - initialNote.start)) };
        }
        return n;
      }));
      return;
    ...
    setNotes((prev) => prev.map((n) => {
      ...
      if (resizeDirection === "right") {
        return { ...n, length: Math.max(1, Math.min(col - n.start + 1, numSteps - n.start)) };
      }
      return n;
    }));
  }, [isResizingLocal, isResizingStore, initialNote, initialMouseX, resizeMode, resizeDirection,
      selectedNoteId, scaleX, numSteps, setNotes, xToContent]);
  ...
  const handleGridClick = useCallback((e) => {
    ...
    if (row < 0 || row >= ROWS || col < 0 || col >= numSteps) return;
  ...
  }, [isResizingStore, mode, numSteps, xToContent, handlePlaySound, setNotes,
    ...
    <div className="bg-gray-900 text-white rounded-xl min-h-0">
      ...
      <div className="flex">
        ...
        <div className="relative w-full overflow-hidden" ref={viewportRef}>
          ...
          <div
            className="relative"
            style={{ width: `${numSteps * CELL_WIDTH}px`, transformOrigin: "left center",
              transform: `translateX(-${startPx}px) scaleX(${scaleX})` }}
          >
            <MeasureLabels cols={numSteps} cellWidth={CELL_WIDTH} />
          </div>
          ...
          <div
            ...
            style={{
              width: `${numSteps * CELL_WIDTH}px`, height: `${ROWS * CELL_HEIGHT}px`,
          ...
          >
            ...
            <GridLines rows={ROWS} cols={numSteps} cellWidth={CELL_WIDTH} cellHeight={CELL_HEIGHT} />
          </div>
        </div>
      </div>
    </div>
